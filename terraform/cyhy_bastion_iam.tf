...
resource "aws_iam_role_policy_attachment" "cloudwatch_agent_policy_attachment_cyhy_bastion" {
  role       = aws_iam_role.cyhy_bastion_instance_role.id
  policy_arn = "arn:aws:iam::aws:policy/CloudWatchAgentServerPolicy"
}

# Attach the SSM Agent policy to this role as well
resource "aws_iam_role_policy_attachment" "ssm_agent_policy_attachment_cyhy_bastion" {
  role       = aws_iam_role.cyhy_bastion_instance_role.id
