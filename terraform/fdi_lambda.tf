...
# cisagov/findings-data-import-lambda project.
data "aws_s3_bucket_object" "fdi_lambda_package" {
  bucket = data.aws_s3_bucket.fdi_lambda.id
  key    = var.findings_data_import_lambda_s3_key
}
...
resource "aws_lambda_function" "fdi_lambda" {
  ...
  runtime       = "python3.9"
  s3_bucket     = data.aws_s3_bucket_object.fdi_lambda_package.bucket
  s3_key        = data.aws_s3_bucket_object.fdi_lambda_package.key
  ...
  source_code_hash = base64sha256(format("%s_%s", data.aws_s3_bucket_object.fdi_lambda_package.key, data.aws_s3_bucket_object.fdi_lambda_package.last_modified))
  timeout          = 300
