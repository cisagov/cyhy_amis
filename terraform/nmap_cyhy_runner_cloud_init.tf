...
data "template_file" "nmap_cyhy_runner_disk_setup" {
  template = "${file("scripts/cyhy_runner_disk_setup.yml")}"

  vars {
    cyhy_runner_disk = "${var.nmap_cyhy_runner_disk}"
  }
}
...
data "template_cloudinit_config" "ssh_and_nmap_cyhy_runner_cloud_init_tasks" {
  ...
  part {
    ...
    content      = "${data.template_file.nmap_cyhy_runner_disk_setup.rendered}"
  }
