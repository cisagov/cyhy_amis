...
resource "null_resource" "bod_bastion_ansible_provisioner" {
  ...
  provisioner "local-exec" {
    command = "ansible-playbook -i '${aws_instance.bod_bastion.public_ip},' ../ansible/playbook.yml --ssh-common-args='-o StrictHostKeyChecking=no' --user=${var.remote_ssh_user} --extra-vars 'host=${aws_instance.bod_bastion.public_ip} host_groups=bod_bastion'"
  }
}
