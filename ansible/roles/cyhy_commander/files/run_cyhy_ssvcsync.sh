...
RELEASE_NAME=$(echo "$RELEASES_JSON" | jq -r '.[].name' | grep -E '^CVE [0-9]{4}-[0-9]{2}-[0-9]{2}_[0-9]{2}00Z$' | head -n 1)
...
ASSET_URL=$(echo "$RELEASES_JSON" | jq -r --arg RELEASE_NAME "$RELEASE_NAME" '.[] | select(.name == $RELEASE_NAME) | .assets[] | select(.name | test("^[0-9]{4}-[0-9]{2}-[0-9]{2}_all_CVEs_at_midnight.zip.zip$")) | .browser_download_url')

# Check if a matching asset was found
if [[ -z "$ASSET_URL" ]]; then
  echo "Error: No asset found in the release '$RELEASE_NAME' matching the expected naming pattern" >&2
  exit 1
fi
